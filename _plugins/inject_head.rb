...
Jekyll::Hooks.register [:pages, :documents], :post_render do |page|
  ...
  injection = <<~HTML
    <style>
      ...
      .cv .list-group-item table{float:left!important;width:200px!important;min-width:200px!important;margin-right:1.5rem!important;border:none!important;border-collapse:collapse!important}
      .cv .list-group-item table td,.cv .list-group-item table th{border:none!important;padding:.1rem 0!important;vertical-align:top!important;background:transparent!important;text-align:left!important;white-space:normal!important;word-break:break-word!important;overflow-wrap:break-word!important}
    ...
    (function(){
      ...
      function expspacing(){
        /* Experience/industry entries: find <strong> labels with em/en dash,
           walk up to containing block, add bottom margin regardless of element type */
        document.querySelectorAll('.cv strong').forEach(function(s){
          var txt=s.textContent;
          if((txt.indexOf('—')>-1||txt.indexOf('–')>-1)&&!s.closest('table')){
            var block=s.closest('p,li,dd,dt,div');
            if(block){
              block.style.setProperty('margin-bottom','1.2rem','important');
              block.style.setProperty('line-height','1.7','important');
            }
          }
        });
      }
      ...
      function run(){colors();typo();btns();cvdates();navname();expspacing();}
